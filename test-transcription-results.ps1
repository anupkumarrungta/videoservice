...
Write-Host "=== Transcription Results Test ===" -ForegroundColor Cyan
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "Existing result files:" -ForegroundColor White
...
try {
    ...
    $testAudioFile = "test-result-audio.mp3"
    ...
    if (Test-Path $testAudioFile) {
        ...
        # Upload to S3
        $s3Key = "transcription/test-result-$(Get-Date -Format 'yyyyMMdd-HHmmss')/test-audio.mp3"
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $jobName = "test-result-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
                ...
                while (-not $completed -and $attempts -lt $maxAttempts) {
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        if ($statusResult -like "*COMPLETED*") {
                            ...
                            Write-Host "Full status result:" -ForegroundColor White
                            ...
                            if ($LASTEXITCODE -eq 0) {
                                ...
                                $downloadResult = aws s3 cp "s3://video-translation-bucket2/$resultKey" test-result.json 2>&1
                                if ($LASTEXITCODE -eq 0) {
                                    Write-Host "✓ Result file downloaded" -ForegroundColor Green
                                    $resultContent = Get-Content test-result.json -Raw
                                    ...
                                    # Clean up local file
                                    Remove-Item test-result.json -ErrorAction SilentlyContinue
                                } else {
                                ...
                                Write-Host "✗ Could not extract transcript URI" -ForegroundColor Red
                                Write-Host "Checking result file directly..." -ForegroundColor Yellow
                                
                                # Always check the result file directly
                                Write-Host "`n3. Checking transcription result file directly..." -ForegroundColor Cyan
                                $resultKey = "transcription-results/$jobName.json"
...
Write-Host "`n=== Transcription Results Test Complete ===" -ForegroundColor Cyan
